...
`static.tildacdn.com`, и на сам домен. Команду выполняет владелец — на маке
или на сервере, из корня репозитория:

```bash
cd clients/remont-keyhome.ru
mkdir -p foto
while read -r u; do
  id=$(echo "$u" | sed -E 's#.*/(tild[^/]+)/.*#\1#')
  ext="${u##*.}"
  curl -sSfL --max-time 30 "$u" -o "foto/$id.$ext" && echo "ок $id.$ext"
done < ФОТО-СПИСОК.txt
du -sh foto && ls foto | wc -l
```

Имя файла берём из адреса (`tild3034-3633-…`), а не из хвоста: на сайте
половина снимков называется `photo_2025-11-26_10-.jpg`, и по именам они
затрут друг друга.

Дальше — `git add clients/remont-keyhome.ru/foto`. Если какие-то отдали
`404`, скажите — значит, ссылка в разметке битая и на живом сайте на этом
месте дырка. Это, кстати, повод для разговора с другом.
