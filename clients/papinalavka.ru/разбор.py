...
def карточки(root: str) -> list[dict]:
    ...
    for fn in sorted(os.listdir(d)):
        ...
        # Пустая карточка — след битого адреса вроде `/view/8_.`: имени нет,
        # цены нет, в каталоге она выглядит строкой из пустоты.
        if not (name and text(name.group(1))):
            continue
        out.append({
            "id": fn[:-5],
...
def отзывы(root: str) -> str:
    ...
    L = ["# Отзывы с сайта papinalavka.ru", "",
         f"Со страницы «О нас»: **{len(revs)} "
         f"{склон(len(revs), 'настоящий отзыв', 'настоящих отзыва', 'настоящих отзывов')}** "
         "с именами и датами. Придумывать для прототипа нечего и нельзя —",
         "здесь есть свои.", "",
