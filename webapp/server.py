...
# Хвост `-01` — номер части: в режиме отправки в GitHub выгрузка режется, и
# недоотправленная часть может остаться на томе. Без него проверка отвергала
# такой файл, и владелец не мог ни скачать его, ни удалить: «Некорректное имя
# архива» на собственный же файл.
_ARCHIVE_RE = re.compile(r"^[a-z0-9.\-]+-\d{4}-\d{2}-\d{2}-\d{4}(-\d{2})?\.zip$", re.I)
