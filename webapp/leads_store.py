    ...
    def all(self) -> dict[str, dict]:
        with self._lock:
            rows = self._conn.execute(
                "SELECT domain, status, note, callback, deal_amount FROM lead_state").fetchall()
        return {d: {"status": s or "", "note": n or "", "callback": cb or "", "deal_amount": da or 0}
                for d, s, n, cb, da in rows}

    def get(self, domain: str) -> dict:
        with self._lock:
            row = self._conn.execute(
                "SELECT status, note, callback, deal_amount FROM lead_state WHERE domain=?", (domain,)
            ).fetchone()
        if not row:
            return {"status": "", "note": "", "callback": "", "deal_amount": 0}
        return {"status": row[0] or "", "note": row[1] or "", "callback": row[2] or "",
                "deal_amount": row[3] or 0}

    def set(self, domain: str, *, status: str | None = None, note: str | None = None,
            callback: str | None = None, deal_amount: float | None = None) -> dict:
        cur = self.get(domain)
        ...
        new_deal = cur["deal_amount"] if deal_amount is None else deal_amount
        with self._lock, self._conn:
            self._conn.execute(
                "INSERT OR REPLACE INTO lead_state(domain, status, note, callback, deal_amount, updated)"
                " VALUES(?,?,?,?,?,?)",
                (domain, new_status, new_note, new_callback, new_deal, time.time()),
            )
        return {"status": new_status, "note": new_note, "callback": new_callback, "deal_amount": new_deal}
