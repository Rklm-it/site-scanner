...
Блоки прототипов лежат **в самом `Caddyfile` в репозитории**, в конце файла —
там же есть готовый образец для `projekt-review.nexus-flow.ru`. Скопируйте его,
поменяйте домен и имя контейнера, закоммитьте.

Дописывать на сервере руками не надо: файл под git, и ручная правка делает
рабочее дерево «грязным». После этого `git pull --rebase` отказывается
работать до `stash` или `checkout` — на этом уже дважды застревали при
обновлении сканера.

Пароля в блоках прототипов **нет намеренно**: ссылку открывает клиент, и
упереться в окно авторизации он не должен. В блоке сканера пароль обязателен —
